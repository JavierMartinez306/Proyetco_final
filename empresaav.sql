...
inner join public.cliente on cliente.cli_cedula = camada.cli_cedula
	...
	--SE INICIA 
	 BEGIN   
		SELECT an_id into estado_malo from estado_del_animal where est_descripcion='REGULAR';
		SELECT cam_id into camada_animal from camada where an_id=estado_malo;
		SELECT cam_id into venta_animal from venta where cam_id=camada_animal;
			if(venta_animal =new.cam_id)then
				raise exception 'Este animal no se puede vender hasta que su estado sea bueno';
			end if;	
		RETURN new;
	 END; 
	...
	CREATE TRIGGER verificar_estado before insert
	on venta for each row 
	execute procedure impedir_venta() ;



INSERT INTO VENTA VALUES (09,2000,'2020-12-14','VENTA DE CAMADA DE CHANCHO BLANCO', 50,1312567876, 1312466145,05);
...
INSERT INTO VENTA VALUES (10,2000,'2020-12-14','VENTA DE CAMADA DE CHANCHO BLANCO', 50,1312567876, 1312466145,05);



----------CURSOR QUE MUESTRE EL NOMBRE DEL CLIENTE, LA FECHA DE COMPRA Y CANTIDAD DE CAMADAS----------------- 
---------------------- ----------------------------
 select * from camada 
	do 
	$$
	DECLARE 
		...
		cur_datos  cursor for SELECT *FROM public.venta 
			INNER JOIN  public.camada ON  venta.cam_id = camada.cam_id 
			INNER JOIN  public.cliente ON camada.cli_cedula = cliente.cli_cedula;
	...
	BEGIN	
		...
		FECHA DE COMPRA: % 
		CANTIDAD DE CAMADAS: %
		RAZA DE ANIMAL COMPRADO: %

		', datos.cli_nombre, datos.ven_fecha, datos.cam_cantidad_animal, datos.cam_descripcion;
		FETCh cur_datos INTO datos;
	end loop;	
	end $$
	Language 'plpgsql';
